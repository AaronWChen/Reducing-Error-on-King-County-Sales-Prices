...
# This Python script is used to import the csv containing King County Real 
# Estate Data into a pandas DataFrame, process the dataframe into a usable form
# with new hybrid columns, applies standard scaling to each column, one hot 
# encoding on the zipcodes, and pickles the resulting dataframes via joblib for
...
def create_train_test_df(ran_state=50):
  # This function imports the raw data csv and splits it into training and test
  # sets with the prices reserved separately as targets
...
def clean_df(X_df):
  # This function takes a dataframe generated by the train/test split and cleans
  # the data by removing the columns not used in the model as well as one 
  # outlier record with 33 bedrooms
...
def distance_calc(cleaned_df):
  # This function takes the cleaned dataframe and uses GeoPy to calculate the 
  # distances between a record and several points of interest. It creates these 
  # distances in list comprehensions that are then added back to the dataframe
  # and then a copy of the original dataframe is returned. 
  #
  # This is the slowest part in the pipeline.
...
def generate_hybrid_data(distance_df):
  # This function takes the dataframe with distances and hybridizes the data to 
  # create meaningful vectors for the model. 
  # 
  # It returns the dataframe with hybridized data columns after dropping the 
  # latitude and longitude values for each record (no longer used due to using 
  # distance)
...
def fit_transform_standard_scale(hybridized):
  # This function performs a fit and transform of standard scaling on each 
  # column in the dataframe, except for zipcode. This allows the data to be 
  # better for the model. It should not be used on test data!
...
def transform_standard_scale(ss, X_test):
  # This function takes the standard scalar object generated from 
  # fit_transform_standard_scale and uses it to transform new data/test data
...
def fit_transform_one_hot_encode_zipcodes(scaled):
  # This function performs a fit and transform of one hot encoding on the 
  # zipcode column. This converts the categorical zipcdes to be used for the 
  # model. It should not be used on test data!
...
def transform_ohe(ohe, scaled):
  # This function takes the OHE object generated from 
  # fit_transform_one_hot_encode_zipcodes and uses it to transform new/test data
...
def make_joblibs(rand_state=50):
  # This function takes a random state (default 50) and uses it to call the 
  # other functions above. After the transformations of the raw data, it will 
  # then take the natural log of the prices as log(price) works better than 
  # regular prices. Afterwards, it pickles the dataframes and prices in joblib 
  # files. The joblib files are used in another script to allow for faster 
  # analysis/compartmentalization of processes.
